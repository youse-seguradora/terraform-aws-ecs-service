...
module "ecs_service" {
  ...
  ecs_task_definition_arn = module.ecs_task_def.arn
  ...
  # Setting app autoscaling target
  resource_id  = "service/${var.ecs_cluster_name}/${var.app_name}"
  max_capacity = 1

  # Settings app autoscaling policy
  name_policy = "${var.app_name}-CPUUtilization"
...
output "ecs_task_def_arm" {
  value = module.ecs_task_def.arn
}
